...
class TargetManagementReferences extends Base
{
	...
	protected function validationTargetReferenceList()
	{
	...
	protected function validationTargetReferenceContentList()
	{
	}



	protected function validationTargetReferenceCreate()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	}
	...
	protected function validationTargetReferenceUpdate()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	...
	protected function validationTargetReferenceDelete()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
		if (isset($this->params['materialCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	}
	...
	public function procTargetReferenceList()
	{
		...
		if ($targetRow == null) {
			...
			return;
                ...
                $materials = TargetManagementUtil::fetchTargetReferenceMaterials($targetCode, $this->getLoginUserCode(), $this->getPDOTarget(), $this->getPDOContents(), $this->siteId);
                $this->response = array('materials' => $materials);
        }



	public function procTargetReferenceCreate()
	{
		$targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		$targetRow = TargetManagementUtil::fetchActiveTargetByCode($targetCode, $this->getLoginUserCode(), $this->getPDOTarget(), $this->getUserInfo($this->getLoginUserCode()), $this->getPDOCommon());
		if ($targetRow == null) {
		...
		if (isset($this->params['linkUrl'])) {
			try {
				...
				Base::writeLog(sprintf('[TargetManagementReferences] URL normalization failed (target:%s link:%s reason:%s)', $targetCode, $sanitizedLink, $exception->getMessage()), 'target-management');
				$this->status = parent::RESULT_ERROR;
				$this->errorReason = 'invalid';
				return;
			}
...
$pdo = $this->getPDOTarget();
		...
		try {
			...
			if ($contentEntry != null) {
				$stmt = $pdo->prepare(
									  'INSERT INTO targetReferenceMaterialContents (materialCode, contentCode, contentType, createdAt) VALUES(?, ?, ?, ?)'
									  );
				$stmt->execute(array(
									 $materialCode,
			...
			$material = $this->fetchReferenceMaterialByCode($materialCode);
	...
	public function procTargetReferenceUpdate()
	{
		$targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		...
		try {
			...
			if ($contentCode !== null && $contentCode !== '') {
				$stmt = $pdo->prepare('INSERT INTO targetReferenceMaterialContents (materialCode, contentCode, contentType, createdAt) VALUES(?, ?, ?, ?)');
				$stmt->execute(array(
					$materialCode,
					$contentCode,
					$contentType,
		...
		$updatedMaterial = $this->fetchReferenceMaterialByCode($materialCode);
	...
	public function procTargetReferenceContentList()
	{
	...
	public function procTargetReferenceDelete()
	{
		$targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		$materialCode = htmlspecialchars($this->params['materialCode'], ENT_QUOTES, "UTF-8");
		...
		try {
			...
			$stmt = $pdo->prepare('UPDATE targetReferenceMaterials SET isDeleted = 1, updatedAt = ? WHERE materialCode = ? AND targetCode = ?');
	...
	private function fetchReferenceMaterialByCode($materialCode)
	{
		...
		if ($row == null) {
			return null;
                ...
                $materials = TargetManagementUtil::buildReferenceMaterialCollection(array($row), $this->getLoginUserCode(), $this->getPDOTarget(), $this->getPDOContents(), $this->siteId);
	...
	private function buildContentMaterialPayload($row)
	{
		...
		$contentRow = array(
                ...
                $payload = TargetManagementUtil::buildReferenceMaterialPayload($materialRow, $this->siteId, $contentRow);
		if ($payload != null) {
