...
class TargetManagementChat extends Base
{
	...
	protected function validationTargetChatThreadList()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	}

	protected function validationTargetChatParticipantProfiles()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
		if (isset($this->params['userCodes']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	}

        protected function validationTargetChatThreadCreate()
        {
                if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
        }

	protected function validationTargetChatMessageCreate()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	...
	protected function validationTargetChatThreadDelete()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
		if (isset($this->params['threadCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	}

	protected function validationTargetChatMessageDelete()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
		if (isset($this->params['threadCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	...
	protected function validationTargetChatThreadMarkRead()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
		if (isset($this->params['threadCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	}

	public function procTargetChatThreadList()
	{
...
$chatData = TargetManagementUtil::fetchTargetChatData(
   $targetCode,
   $viewerUserCode,
   array(
 ...
 'threadCodes' => $requestedThreadCodes,
   ...
   $chatDependencies
		...
		$threads = isset($chatData['threads']) && is_array($chatData['threads']) ? $chatData['threads'] : array();
		$participants = isset($chatData['participants']) && is_array($chatData['participants']) ? $chatData['participants'] : array();
		$pagination = isset($chatData['pagination']) && is_array($chatData['pagination']) ? $chatData['pagination'] : array();
	...
	public function procTargetChatParticipantProfiles()
	{
		$targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		if ($targetCode === '') {
			$this->status = parent::RESULT_ERROR;
...
$chatDependencies = $this->buildChatUtilDependencies();
$chatData = TargetManagementUtil::fetchTargetChatData($targetCode, $loginUserCode, array(), $chatDependencies);
		...
		foreach ($userCodes as $normalized => $originalCode) {
			...
			$participant = $this->buildChatParticipantPayload($originalCode, null);
			if ($participant == null) {
				continue;
			}
        ...
        public function procTargetChatThreadCreate()
        {
                $targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		...
		$recipientCodes = $this->parseChatRecipientCodes($recipientCodesRaw);
		...
		$threadTypeParam = isset($this->params['threadType']) ? $this->params['threadType'] : null;
                $threadType = $this->normalizeChatThreadTypeValue($threadTypeParam, count($filteredRecipients));
                ...
                if ($contentValue !== null) {
                        $lastMessageAt = $now;
                        $lastMessageSnippet = $this->createChatSnippet($contentValue);
                ...
                try {
                        ...
                        $stmt = $pdo->prepare('INSERT INTO targetChatThreads (threadCode, targetCode, threadType, title, description, createdByUserCode, createdAt, updatedAt, lastMessageAt, lastMessageSnippet, lastMessageSenderCode, isArchived, isLocked) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0)');
                        ...
                        $memberStmt = $pdo->prepare('INSERT OR IGNORE INTO targetChatThreadMembers (threadCode, userCode, joinedAt, notificationsMuted) VALUES (?, ?, ?, 0)');
                        $memberStmt->execute(array($threadCode, $loginUserCode, $now));
                        foreach ($filteredRecipients as $code) {
                                $memberRole = $this->deriveChatMemberRole($targetRow, $code);
                        ...
                        if ($contentValue !== null) {
                                ...
                                $messageStmt = $pdo->prepare('INSERT INTO targetChatMessages (messageCode, threadCode, senderUserCode, content, sentAt, deliveredAt, readAt, createdAt, updatedAt, replyToMessageCode, metadata, isDeleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, 0)');
                                $messageStmt->execute(array($messageCode, $threadCode, $loginUserCode, $contentValue, $now, $now, $now, $now, $now, $metadata));

                                $readStmt = $pdo->prepare('INSERT OR REPLACE INTO targetChatMessageReads (messageCode, userCode, readAt, createdAt) VALUES (?, ?, ?, ?)');
                                $readStmt->execute(array($messageCode, $loginUserCode, $now, $now));

                                $updateStmt = $pdo->prepare('UPDATE targetChatThreads SET lastMessageAt = ?, lastMessageSnippet = ?, lastMessageSenderCode = ?, updatedAt = ? WHERE threadCode = ?');
		...
		$this->buildChatThreadResponse($targetCode, $threadCode);
	}



	public function procTargetChatMessageCreate()
	{
                $targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		...
		if ($targetRow == null) {
			...
			return;
                ...
                $pdo = $chatDependencies['pdo'];
		$stmt = $pdo->prepare('SELECT threadCode, targetCode, threadType FROM targetChatThreads WHERE threadCode = ? AND (isArchived IS NULL OR isArchived = 0) LIMIT 1');
		...
		if ($loginUserCode === null || $loginUserCode === '') {
			...
			return;
                ...
                if ($senderOverrideSpecified) {
                        $memberCheck = $pdo->prepare('SELECT 1 FROM targetChatThreadMembers WHERE threadCode = ? AND userCode = ? LIMIT 1');
                ...
                $now = date('Y-m-d H:i:s');
                $messageCode = $this->generateUniqid();
                $snippet = $this->createChatSnippet($contentValue);
                $metadata = json_encode(array('emphasis' => 'normal'));
		if ($metadata === false) {
			$metadata = '{}';
		}
	...
	public function procTargetChatThreadMarkRead()
	{
		$targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		$threadCode = htmlspecialchars($this->params['threadCode'], ENT_QUOTES, "UTF-8");
		...
		if ($targetRow == null) {
			...
			return;
                ...
                $pdo = $chatDependencies['pdo'];
		$stmt = $pdo->prepare('SELECT threadCode, targetCode FROM targetChatThreads WHERE threadCode = ? AND (isArchived IS NULL OR isArchived = 0) LIMIT 1');
		$stmt->execute(array($threadCode));
		$threadRow = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		$messageStmt = $pdo->prepare('SELECT messageCode FROM targetChatMessages WHERE threadCode = ? AND (isDeleted IS NULL OR isDeleted = 0)');
		...
		if (count($messageCodes) > 0) {
			...
			try {
				...
				$existingStmt = $pdo->prepare('SELECT messageCode, createdAt FROM targetChatMessageReads WHERE userCode = ? AND messageCode IN (' . $placeholders . ')');
	...
	public function procTargetChatThreadDelete()
	{
		$targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		$threadCode = htmlspecialchars($this->params['threadCode'], ENT_QUOTES, "UTF-8");
		...
		if ($this->userCanAccessTarget($targetRow, $targetCode, $loginUserCode) == false && $this->isSupervisor() == false && $this->isOperator() == false) {
			...
			return;
                ...
                $pdo = $chatDependencies['pdo'];
		$threadStmt = $pdo->prepare('SELECT threadCode, targetCode, createdByUserCode FROM targetChatThreads WHERE threadCode = ? AND (isArchived IS NULL OR isArchived = 0) LIMIT 1');
		...
		if ($canDelete == false) {
			$firstMessageStmt = $pdo->prepare('SELECT senderUserCode FROM targetChatMessages WHERE threadCode = ? AND (isDeleted IS NULL OR isDeleted = 0) ORDER BY sentAt ASC, id ASC LIMIT 1');
		...
		$messageCodes = array();
		$messageCodeStmt = $pdo->prepare('SELECT messageCode FROM targetChatMessages WHERE threadCode = ?');
		...
		try {
			...
			if (count($messageCodes) > 0) {
				$placeholders = implode(', ', array_fill(0, count($messageCodes), '?'));
				$attachmentStmt = $pdo->prepare('DELETE FROM targetChatMessageAttachments WHERE messageCode IN (' . $placeholders . ')');
				$attachmentStmt->execute($messageCodes);

				$readStmt = $pdo->prepare('DELETE FROM targetChatMessageReads WHERE messageCode IN (' . $placeholders . ')');
				$readStmt->execute($messageCodes);
			}

			$deleteMessagesStmt = $pdo->prepare('UPDATE targetChatMessages SET isDeleted = 1, updatedAt = ? WHERE threadCode = ?');
			$deleteMessagesStmt->execute(array($now, $threadCode));

			$archiveThreadStmt = $pdo->prepare('UPDATE targetChatThreads SET isArchived = 1, lastMessageAt = NULL, lastMessageSnippet = NULL, lastMessageSenderCode = NULL, updatedAt = ? WHERE threadCode = ?');
	...
	public function procTargetChatMessageDelete()
	{
		$targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		$threadCode = htmlspecialchars($this->params['threadCode'], ENT_QUOTES, "UTF-8");
		...
		if ($this->userCanAccessTarget($targetRow, $targetCode, $loginUserCode) == false && $this->isSupervisor() == false && $this->isOperator() == false) {
			...
			return;
                ...
                $pdo = $chatDependencies['pdo'];
		$threadStmt = $pdo->prepare('SELECT threadCode, targetCode FROM targetChatThreads WHERE threadCode = ? AND (isArchived IS NULL OR isArchived = 0) LIMIT 1');
		$threadStmt->execute(array($threadCode));
		$threadRow = $threadStmt->fetch(PDO::FETCH_ASSOC);
		...
		$messageStmt = $pdo->prepare('SELECT messageCode, threadCode, senderUserCode FROM targetChatMessages WHERE messageCode = ? AND (isDeleted IS NULL OR isDeleted = 0) LIMIT 1');
		...
		try {
			...
			$deleteAttachmentsStmt = $pdo->prepare('DELETE FROM targetChatMessageAttachments WHERE messageCode = ?');
			$deleteAttachmentsStmt->execute(array($messageCode));

			$deleteReadsStmt = $pdo->prepare('DELETE FROM targetChatMessageReads WHERE messageCode = ?');
			$deleteReadsStmt->execute(array($messageCode));

			$deleteMessageStmt = $pdo->prepare('UPDATE targetChatMessages SET isDeleted = 1, updatedAt = ? WHERE messageCode = ?');
			$deleteMessageStmt->execute(array($now, $messageCode));

			$lastMessageStmt = $pdo->prepare('SELECT sentAt, createdAt, updatedAt, content, senderUserCode FROM targetChatMessages WHERE threadCode = ? AND (isDeleted IS NULL OR isDeleted = 0) ORDER BY COALESCE(sentAt, createdAt, updatedAt) DESC, id DESC LIMIT 1');
			...
			if ($lastMessageRow != false) {
				...
				$lastSnippet = $this->createChatSnippet(isset($lastMessageRow['content']) ? $lastMessageRow['content'] : null);
				if (isset($lastMessageRow['senderUserCode'])) {
					$lastSender = $lastMessageRow['senderUserCode'];
				}
			}

			$updateThreadStmt = $pdo->prepare('UPDATE targetChatThreads SET lastMessageAt = ?, lastMessageSnippet = ?, lastMessageSenderCode = ?, updatedAt = ? WHERE threadCode = ?');
        ...
        public function buildChatParticipantPayload($userCode, $role)
        {
        ...
        public function buildChatAttachmentPayload($row)
        {
                if (!is_array($row)) {
                        return null;
                }
	...
	private function parseChatRecipientCodes($value)
	{
		if (is_string($value)) {
			$trimmed = trim($value);
			if ($trimmed === '') {
	...
	private function normalizeChatThreadTypeValue($value, $recipientCount)
	{
	...
	private function deriveChatMemberRole($targetRow, $userCode)
	{
		if ($userCode === null || $userCode === '') {
			return 'member';
		}
        ...
        private function createChatSnippet($content)
        {
                if ($content === null) {
			return null;
		}
	...
	private function buildChatThreadResponse($targetCode, $threadCode)
	{
$viewerUserCode = $this->getLoginUserCode();
$fetchOptions = array();
...
$chatDependencies = $this->buildChatUtilDependencies();
$chatData = TargetManagementUtil::fetchTargetChatData($targetCode, $viewerUserCode, $fetchOptions, $chatDependencies);
		$rawThreads = isset($chatData['threads']) && is_array($chatData['threads']) ? $chatData['threads'] : array();
		$threads = array();
		...
		$rawParticipants = isset($chatData['participants']) && is_array($chatData['participants']) ? $chatData['participants'] : array();
