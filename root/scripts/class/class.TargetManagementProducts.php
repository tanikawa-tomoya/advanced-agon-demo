...
class TargetManagementProducts extends Base
{
	...
	protected function validationTargetProductList()
	{
	...
	protected function validationTargetProductContentList()
	{
	}



	protected function validationTargetProductCreate()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	}
	...
	protected function validationTargetProductUpdate()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	...
	protected function validationTargetProductDelete()
	{
		if (isset($this->params['targetCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
		if (isset($this->params['materialCode']) == false) { throw new Exception(__FILE__ . ":" . __LINE__); }
	}
	...
	public function procTargetProductList()
	{
		...
		if ($targetRow == null) {
			...
			return;
                ...
                $materials = TargetManagementUtil::fetchTargetProductMaterials($targetCode, $this->getLoginUserCode(), $this->getPDOTarget(), $this->getPDOContents(), $this->siteId);
                $this->response = array('materials' => $materials);
        }



	public function procTargetProductCreate()
	{
		$targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		$targetRow = TargetManagementUtil::fetchActiveTargetByCode($targetCode, $this->getLoginUserCode(), $this->getPDOTarget(), $this->getUserInfo($this->getLoginUserCode()), $this->getPDOCommon());
		if ($targetRow == null) {
		...
		if (isset($this->params['linkUrl'])) {
			try {
				...
				Base::writeLog(sprintf('[TargetManagementProducts] URL normalization failed (target:%s link:%s reason:%s)', $targetCode, $sanitizedLink, $exception->getMessage()), 'target-management');
				$this->status = parent::RESULT_ERROR;
				$this->errorReason = 'invalid';
				return;
			}
		...
		if ($contentEntry !== null) {
			...
			if ($contentType === null && isset($contentEntry['contentType'])) {
                ...
                // Product material records themselves live in the target database,
		...
		try {
			$pdo->beginTransaction();
                        ...
                        $stmt = $pdo->prepare(
                                                                  'INSERT INTO targetProductMaterials (materialCode, targetCode, contentCode, title, description, startDate, endDate, category, linkUrl, downloadUrl, fileName, fileSize, ownerUserCode, createdAt, updatedAt, displayOrder, isDeleted) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0)'
                                                                  );
                        $stmt->execute(array(
                                                                 $materialCode,
                                                                 $targetCode,
			...
			if ($contentEntry != null) {
				$stmt = $pdo->prepare(
									  'INSERT INTO targetProductMaterialContents (materialCode, contentCode, contentType, createdAt) VALUES(?, ?, ?, ?)'
									  );
				$stmt->execute(array(
									 $materialCode,
			...
			$material = $this->fetchProductMaterialByCode($materialCode);
	...
	public function procTargetProductUpdate()
	{
		$targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		...
		$pdo = $this->getPDOTarget();
                try {
                        ...
                        $stmt = $pdo->prepare('UPDATE targetProductMaterials SET contentCode = ?, title = ?, description = ?, startDate = ?, endDate = ?, category = ?, linkUrl = ?, downloadUrl = ?, fileName = ?, fileSize = ?, ownerUserCode = ?, updatedAt = ? WHERE materialCode = ? AND targetCode = ?');
                        $stmt->execute(array(
                                $contentCode,
                                $titleValue,
                                $description,
			...
			$stmt = $pdo->prepare('DELETE FROM targetProductMaterialContents WHERE materialCode = ?');
			$stmt->execute(array($materialCode));

			if ($contentCode !== null && $contentCode !== '') {
				$stmt = $pdo->prepare('INSERT INTO targetProductMaterialContents (materialCode, contentCode, contentType, createdAt) VALUES(?, ?, ?, ?)');
				$stmt->execute(array(
					$materialCode,
					$contentCode,
					$contentType,
		...
		$updatedMaterial = $this->fetchProductMaterialByCode($materialCode);
	...
	public function procTargetProductContentList()
	{
	...
	public function procTargetProductDelete()
	{
		$targetCode = htmlspecialchars($this->params['targetCode'], ENT_QUOTES, "UTF-8");
		$materialCode = htmlspecialchars($this->params['materialCode'], ENT_QUOTES, "UTF-8");
		...
		try {
			...
			$stmt = $pdo->prepare('UPDATE targetProductMaterials SET isDeleted = 1, updatedAt = ? WHERE materialCode = ? AND targetCode = ?');
	...
	private function fetchProductMaterialByCode($materialCode)
	{
		...
		if ($row == null) {
			return null;
                ...
                $materials = TargetManagementUtil::buildProductMaterialCollection(array($row), $this->getLoginUserCode(), $this->getPDOTarget(), $this->getPDOContents(), $this->siteId);
