	...
	public function procSubmissionDetail()
	{
        ...
        private function fetchSubmissionContents($submissionCode)
        {
                ...
                if (count($contentCodes) > 0) {
                        ...
                        $contentsStmt = $this->getPDOContents()->prepare('SELECT id, contentCode, userCode, contentType, fileName, description, filePath, mimeType, fileSize, duration, bitrate, width, height, createdAt, updatedAt FROM userContents WHERE contentCode IN (' . $placeholders . ')');
