(function ()
{
  ...
  class JobModal
  {
    ...
    ensureElements()
    {
      ...
      modal.dataset.modal = 'target-select-modal';
      ...
      dialog.setAttribute('aria-labelledby', 'target-select-modal-title');
      dialog.setAttribute('aria-describedby', 'target-select-modal-summary');
    ...
    renderTable(container, items, context)
    {
      ...
      var headers = ['タイトル', 'コード', 'ステータス', 'カテゴリー'];
      ...
      items.forEach((item) =>
      {
        ...
        codeCell.textContent = item.targetCode || item.userCode || '—';
        row.appendChild(codeCell);

        var mailCell = document.createElement('td');
        mailCell.textContent = item.status || '—';
        row.appendChild(mailCell);

        var groupCell = document.createElement('td');
        groupCell.textContent = item.category || '—';
        ...
        if (context.multiple)
        {
          ...
          var identityText = item.title || item.targetCode || item.displayName || 'ターゲット';
    ...
    open(options)
    {
      ...
      var availableTargets = Array.isArray(options.availableTargets)
      ...
      if (availableTargets)
      {
        ...
        {
          handleLoaded(availableTargets);
        });
      }
      else
      {
        this.service.jobs.data.getAllTargets(loadOptions)
          .then((list) =>
          {
            handleLoaded(list);
          })
