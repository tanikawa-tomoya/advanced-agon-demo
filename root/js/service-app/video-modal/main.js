(function () {
  ...
  class VideoModalService
  {
    ...
    initConfig(options)
    {
      ...
      var CSS = {
        ...
        player: 'c-video-modal__player',
        loading: 'c-video-modal__loading',
        loadingSpinner: 'c-video-modal__loading-spinner',
        loadingLabel: 'c-video-modal__loading-label',
    ...
    async boot()
    {
      await window.Utils.loadScriptsSync([
        'js/service-app/video-modal/job-embed.js',
    ...
    _createContentNode(spec, opts) {
      ...
      if (provider === 'youtube') {
        ...
        if (!yid) throw new Error('video-modal: invalid YouTube id/url');
      ...
      if (provider === 'vimeo') {
        ...
        if (!vid) throw new Error('video-modal: invalid Vimeo id/url');
    ...
    async openContentVideo(spec, options)
    {
      ...
      if (!fallbackSrc)
      {
        throw new Error('video-modal: contentCode is required');
      }
  ...
  window.Services.VideoModal = VideoModalService;
