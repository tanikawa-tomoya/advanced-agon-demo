  ...
  class JobBitrate
  {
    ...
    bindActions(container, videoEl, variants, initialKey, autoplay)
    {
      ...
      var handler = function (event)
      {
        ...
        var loadingOverlay = self.showLoading(videoEl);
        currentKey = key;
        self.updateActive(container, key);
        self.switchSource(videoEl, variant, autoplay, loadingOverlay);
      };
    ...
    ensureLoadingOverlay(videoEl)
    {
      var host = (videoEl && videoEl.parentNode) ? videoEl.parentNode : null;
      if (!host)
      {
        return null;
      }
      var existing = host.querySelector('.' + this.service.CSS.loading);
      if (existing)
      {
        return existing;
      }
      var overlay = document.createElement('div');
      overlay.className = this.service.CSS.loading;
      var spinner = document.createElement('div');
      spinner.className = this.service.CSS.loadingSpinner;
      spinner.setAttribute('aria-hidden', 'true');
      var label = document.createElement('div');
      label.className = this.service.CSS.loadingLabel;
    ...
    showLoading(videoEl)
    {
      var overlay = this.ensureLoadingOverlay(videoEl);
      if (overlay)
      {
    ...
    switchSource(videoEl, variant, autoplay, loadingOverlay)
    {
      var currentTime = 0;
      ...
      var cleanup = function ()
      {
        ...
        handled = true;
        self.hideLoading(loadingOverlay);
