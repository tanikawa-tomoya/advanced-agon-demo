(function () {
  ...
  class CanvasVideoModalService
  {
    ...
    async boot()
    {
      await window.Utils.loadScriptsSync([
        'js/service-app/canvas-video-modal/job-embed.js',
        'js/service-app/canvas-video-modal/job-modal.js',
        'js/service-app/canvas-video-modal/job-content.js',
        'js/service-app/canvas-video-modal/job-bitrate.js',
      ...
      this.jobs = {
        embed: new window.Services.CanvasVideoModal.JobEmbed(this),
        modal: new window.Services.CanvasVideoModal.JobModal(this),
        content: new window.Services.CanvasVideoModal.JobContent(this),
        bitrate: new window.Services.CanvasVideoModal.JobBitrate(this, this.buttonService)
      };
      return this;
    }
    ...
    _createContentNode(spec, opts) {
      ...
      if (provider === 'youtube') {
        ...
        if (!yid) throw new Error('canvas-video-modal: invalid YouTube id/url');
        return { node: this.jobs.embed.buildYouTubeIframe(yid, !!opts.autoplay, spec.title || 'YouTube', this.config.iframeAllow) };
      }
      if (provider === 'vimeo') {
        var vid = this.jobs.embed.parseVimeoId(spec.id || spec.url || '');
        if (!vid) throw new Error('canvas-video-modal: invalid Vimeo id/url');
        return { node: this.jobs.embed.buildVimeoIframe(vid, !!opts.autoplay, spec.title || 'Vimeo', this.config.iframeAllow) };
      }
      if (provider === 'html5') {
        var safe = this.jobs.embed.sanitizeUrl(spec.src || '', this.ALLOWED_IFRAME_HOSTS);
        if (!safe) throw new Error('canvas-video-modal: src must be https or relative');
        return this.jobs.embed.buildCanvasVideoPlayer(safe, !!opts.autoplay);
      }
      throw new Error('canvas-video-modal: unsupported provider: ' + provider);
    }
     ...
     */
    show(spec, options) {
      if (!spec) throw new Error('canvas-video-modal: spec required');
      var opts = Object.assign({}, this.config, options || {});
      var content = this._createContentNode(spec, opts) || {};
      var contentNode = content.node || content;
      var disposeHandlers = [];
      if (typeof content.dispose === 'function')
      {
        disposeHandlers.push(content.dispose);
      }
    ...
    async openContentVideo(spec, options)
    {
      ...
      if (!fallbackSrc)
      {
        throw new Error('canvas-video-modal: contentCode is required');
      }
      ...
      var player = this.jobs.embed.buildCanvasVideoPlayer(initialVariant.src, !!opts.autoplay);
      var videoEl = player.video;
      var actionsNode = this.jobs.bitrate.createActions(variants, initialVariant.key);
      var disposeHandlers = [];
      if (player && typeof player.dispose === 'function')
      {
        disposeHandlers.push(player.dispose);
      }
      if (actionsNode)
      {
      ...
      var modalEl = this.jobs.modal.createModal({
        CSS: this.CSS,
        title: title,
        contentNode: player && player.node ? player.node : videoEl,
  ...
  window.Services.CanvasVideoModal = CanvasVideoModalService;
