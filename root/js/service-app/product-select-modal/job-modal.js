(function ()
{
  ...
  class JobModal
  {
    ...
    ensureElements()
    {
      ...
      modal.dataset.modal = 'product-select-modal';
      ...
      dialog.setAttribute('aria-labelledby', 'product-select-modal-title');
      dialog.setAttribute('aria-describedby', 'product-select-modal-summary');
    ...
    renderTable(container, items, context)
    {
      ...
      items.forEach((item) =>
      {
        ...
        ownerCell.textContent = item.ownerDisplayName || item.ownerUserCode || '—';
        ...
        if (context.multiple)
        {
          ...
          var identityText = item.title || item.materialCode || item.displayName || '商品資料';
    ...
    open(options)
    {
      ...
      var availableMaterials = Array.isArray(options.availableMaterials)
        ? options.availableMaterials
        : (Array.isArray(options.availableTargets)
          ? options.availableTargets
          : (Array.isArray(options.availableUsers) ? options.availableUsers : null));
      var loadOptions = options.loadOptions || {};
      if (!loadOptions.targetCode && options.targetCode)
      {
        loadOptions.targetCode = options.targetCode;
      }
      ...
      var normalizeId = function (item)
      {
        ...
        var id = item.materialCode || item.code || item.contentCode || '';
      ...
      if (availableMaterials)
      {
        ...
        {
          handleLoaded(availableMaterials);
        });
      }
      else
      {
        this.service.jobs.data.getAllMaterials(loadOptions)
          .then((list) =>
          {
            handleLoaded(list);
          })
