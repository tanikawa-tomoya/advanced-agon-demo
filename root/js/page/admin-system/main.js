(function (window, document) {
  ...
  class AdminSystem
  {
    ...
    {
      this.selectorConfig = {
        root: '#system-settings-root',
        tabRoot: '[data-tab-root]',
        tabTriggers: '[data-tab]',
        tabPanels: '[data-tab-panel]',
        settingsTableBody: '#system-settings-table-body',
        settingsRefresh: '#system-settings-refresh',
        storageTotal: '#system-storage-total',
        storageUserDatas: '#system-storage-userDatas',
        storageRefresh: '#system-storage-refresh',
        softwareTableBody: '#system-software-table-body',
        softwareRefresh: '#system-software-refresh',
        helpButton: '#system-management-help-button',
        helpModal: '#system-management-help-modal'
      };
      this.apiConfig = {
    ...
    async _loadCoreServices()
    {
      ...
      this.loadingService = new window.Services.Loading({ container: document.querySelector('#queue-root') || document.body });
    ...
    {
      const queue = [
        { src: '/js/page/admin-system/job-tabs.js', exportName: 'JobTabs' },
        { src: '/js/page/admin-system/job-settings.js', exportName: 'JobSettings' },
        { src: '/js/page/admin-system/job-storage.js', exportName: 'JobStorage' },
        { src: '/js/page/admin-system/job-software.js', exportName: 'JobSoftware' },
        { src: '/js/page/admin-system/job-help.js', exportName: 'JobHelp' }
      ];

      for (let i = 0; i < queue.length; i += 1) {
        const meta = queue[i];
        await window.Utils.loadScriptsSync([{ src: meta.src }]);
        const namespace = window.AdminSystem || {};
    ...
    decorateRefreshButton(placeholder, options)
    {
      if (!placeholder) {
        return null;
      }
      const opts = options || {};
      const buttonType = opts.buttonType || 'expandable-icon-button/reload';
      const label = opts.label || placeholder.getAttribute('data-label') || placeholder.textContent || '再読み込み';
      ...
      Object.keys(placeholderDataset).forEach(function (key) {
        ...
        if (dataset[key] === undefined) {
          dataset[key] = placeholderDataset[key];
        }
      });
      ...
      if (opts.baseClass === undefined) {
        opts.baseClass = 'target-management__icon-button target-management__icon-button--ghost system-refresh-button';
      }
      let button = null;
      ...
      if (!button) {
        ...
        placeholder.classList.add('system-refresh-button');
  ...
  window.AdminSystem = window.AdminSystem || AdminSystem;
