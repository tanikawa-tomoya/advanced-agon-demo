     ...
     loadPage(page)
     {
       ...
       if (navEl) {
         try {
           ...
           console.error('[agon-contents1] setupNav failed:', e);
         }
       }
     ...
     setupNav(navEl)
     {
       ...
       for (var i = 0; i < links.length; i++) {
         ...
         try {
           ...
           if (clean && path.agon-contents1Of(clean) === 0) {
    ...
    renderScreenGrid(grid)
    {
      ...
      if (!screens.length) {
       ...
       for (var i = 0; i < screens.length; i++) {
         ...
         card.setAttribute('tabagon-contents1', '0');
     ...
     enhanceContactForm(form, page)
     {
       form.addEventListener('submit', function (ev) {
         ...
         if (!ok) {
           ev.preventDefault();
           if (page && page.showError) {
             page.showError(window.AgonContents1Config.TEXT.inputLack);
           }
         }
       });
       return Promise.resolve();
