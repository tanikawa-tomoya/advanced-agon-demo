(function (window, document)
{
  ...
  class TargetDetailProduct
  {
    ...
    async render()
    {
      ...
      this.refs.container = this.page.refs.tabPanels && this.page.refs.tabPanels.products;
      ...
      list.className = 'target-product__list';
      this.refs.list = list;
      ...
      var data = await this.page.loadProducts();
    ...
    renderHeader()
    {
      ...
      actions.className = 'target-detail__section-actions target-product__actions target-product__header-actions';
      ...
      var refreshButton = this.createBannerButton('再読み込み', 'target-product__refresh', {
    ...
    renderFilters()
    {
      ...
      filters.className = 'target-product__controls target-product__filters';
    ...
    renderFeedbackRegion()
    {
      ...
      region.className = 'c-feedback-region target-product__feedback';
    ...
    async reloadProductsFromServer()
    {
      var data = await this.page.loadProducts({ force: true });
      this.disposeObjectUrls(this.state.items);
      this.state.items = this.normalizeMaterials(Array.isArray(data) ? data : []);
      this.updateFilterOptions();
      this.renderList();
    ...
    normalizeMaterial(raw)
    {
      ...
      var code = raw.materialCode || raw.contentCode || raw.productCode || raw.id;
      if (!code)
      {
    ...
    renderList()
    {
      ...
      table.className = 'target-product__table target-detail__submission-table';
      ...
      thead.innerHTML = '' +
        ...
        '<th scope="col">リンク</th>' +
        '<th scope="col">更新日</th>' +
        '<th scope="col" class="target-product__actions-header">操作</th>' +
        '</tr>';
    ...
    createTableRow(item)
    {
      ...
      infoCell.className = 'target-product__info-cell';
      var titleRow = document.createElement('div');
      titleRow.className = 'target-product__title-row';

      var titleWrapper = document.createElement('div');
      titleWrapper.className = 'target-product__title-wrapper';

      var title = document.createElement('p');
      title.className = 'target-product__item-title';
      ...
      if (item.description)
      {
        ...
        description.className = 'target-product__description';
      ...
      updatedCell.className = 'target-product__update-cell';
      ...
      var previewButton = this.createRoundButton('detail', 'プレビュー', 'target-product__action target-product__preview', {
        hoverLabel: 'プレビューを表示',
        title: 'プレビューを表示'
      });
      ...
      var downloadButton = this.createRoundButton('download', 'ダウンロード', 'target-product__action target-product__download', {
    ...
    createThumbnailCell(item)
    {
      ...
      cell.className = 'target-product__thumbnail-cell';

      var wrapper = document.createElement('div');
      wrapper.className = 'target-product__thumbnail';
      ...
      else
      {
        ...
        placeholder.className = 'target-product__thumbnail-placeholder';
    ...
    createPdfThumbnailPlaceholder(item)
    {
      ...
      host.className = 'target-product__thumbnail-emboss target-detail__guidance-thumb-emboss-host';
    ...
    {
      var cell = document.createElement('td');
      cell.className = 'target-product__owner-cell target-detail__submission-user-cell';
    ...
    {
      var cell = document.createElement('td');
      cell.className = 'target-product__period-cell';
    ...
    {
      var cell = document.createElement('td');
      cell.className = 'target-product__link-cell';
      ...
      anchor.className = 'target-product__link';
    ...
    createOwnerDisplay(item)
    {
      ...
      anchor.tabIndex = 0;
      anchor.setAttribute('data-product-avatar-anchor', 'true');
      ...
      if (avatarService && typeof avatarService.render === 'function')
      {
        ...
        catch (error)
        {
          ...
          if (window.console && typeof window.console.warn === 'function')
          {
            window.console.warn('[target-detail][product] failed to render owner avatar', error);
          }
        }
      }
      if (!avatarRendered)
    ...
    bindOwnerPopovers(container)
    {
      ...
      var anchors = container.querySelectorAll('[data-product-avatar-anchor]');
      if (!anchors.length)
      {
        return;
      }
    ...
    async handleDelete(item, button)
    {
      ...
      try
      {
        await this.page.callApi('TargetProductDelete', {
          targetCode: targetCode,
          materialCode: item.materialCode
        }, {
          requestType: 'TargetManagementProducts'
        });
        ...
        if (Array.isArray(this.page.state.products))
        {
          this.page.state.products = this.page.state.products.filter(function (entry)
          {
            return entry && entry.materialCode !== item.materialCode;
          });
        }
    ...
    createAddModal()
    {
      ...
      modalRoot.className = 'screen-modal target-product__modal-container';
      ...
      modalRoot.innerHTML =
        ...
        '<label class="target-product__form-label" for="target-product-title">タイトル</label>' +
        '<input id="target-product-title" class="user-management__input target-product__input" name="title" required maxlength="256" />' +
        '</div>' +
        '<div class="target-product__form-field target-product__form-row--full">' +
        '<label class="target-product__form-label" for="target-product-description">説明 (任意)</label>' +
        '<textarea id="target-product-description" class="user-management__input target-product__textarea" name="description" rows="3" maxlength="2048"></textarea>' +
        '</div>' +
        '<div class="target-product__form-row target-product__form-dates">' +
        '<div class="target-product__form-field">' +
        '<label class="target-product__form-label" for="target-product-start">開始日 (任意)</label>' +
        '<input id="target-product-start" class="user-management__input target-product__input" name="startDate" type="date" />' +
        '</div>' +
        '<div class="target-product__form-field">' +
        '<label class="target-product__form-label" for="target-product-end">終了日 (任意)</label>' +
        '<input id="target-product-end" class="user-management__input target-product__input" name="endDate" type="date" />' +
        '</div>' +
        '</div>' +
        '<div class="target-product__form-field target-product__form-row--full">' +
        '<label class="target-product__form-label" for="target-product-link">リンクURL (任意)</label>' +
        '<input id="target-product-link" class="user-management__input target-product__input" name="linkUrl" type="url" placeholder="https://example.com/event" />' +
        '<p class="target-product__form-hint">リンクのみで登録する場合はファイル選択は不要です。</p>' +
        '</div>' +
        '<div class="target-product__form-field">' +
        '<span class="target-product__form-label">作成者</span>' +
        '<div class="target-product__author" data-target-product-author-picker>' +
        '<p class="target-product__author-empty" data-target-product-author-empty>作成者が選択されていません。</p>' +
        '<div class="target-product__author-summary" data-target-product-author-summary hidden>' +
        '<span class="target-product__author-name" data-target-product-author-name></span>' +
        '<span class="target-product__author-code" data-target-product-author-code></span>' +
        '</div>' +
        '</div>' +
        '<div class="target-product__form-actions target-product__author-actions" data-target-product-author-actions></div>' +
        '</div>' +
        '<div class="target-submission__panel-grid">' +
        '<section class="target-submission__panel" aria-labelledby="target-product-panel-library">' +
        '<div class="target-product__form-field target-product__form-row--full">' +
        '<div class="target-submission__content-select-header">' +
        '<p class="target-product__form-label" id="target-product-panel-library">登録済みコンテンツから追加</p>' +
        '</div>' +
        '<div class="target-submission__content-select-box">' +
        '<p class="target-product__upload-note" data-target-product-content-summary>コンテンツを選択していません。</p>' +
        '<div class="target-product__form-actions target-submission__content-select-actions">' +
        '<button type="button" class="btn btn--primary" data-target-product-content-open>コンテンツを選択</button>' +
        '<button type="button" class="btn btn--text" data-target-product-content-clear disabled>選択をクリア</button>' +
        '</div>' +
        '</div>' +
        '<ul class="target-submission__content-list" data-target-product-content-list hidden></ul>' +
        '<div class="target-submission__content-picker" data-target-product-content-picker hidden>' +
        '<label class="target-product__form-label" for="target-product-content-search">コンテンツを検索</label>' +
        '<input id="target-product-content-search" class="user-management__input target-product__input" type="search" placeholder="タイトルやコンテンツコードで検索" data-target-product-content-search />' +
        '<p class="target-product__upload-note" data-target-product-content-feedback hidden></p>' +
        '<div class="target-submission__content-picker-results" data-target-product-content-results></div>' +
        '<div class="target-product__form-actions target-submission__content-picker-actions">' +
        '<button type="button" class="btn btn--ghost" data-target-product-content-close>閉じる</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</section>' +
        '<section class="target-submission__panel" aria-labelledby="target-product-panel-upload">' +
        '<div class="target-product__form-field target-product__form-row--full target-product__form-upload">' +
        '<p class="target-product__form-label" id="target-product-panel-upload">コンテンツをアップロード</p>' +
        '<div class="target-product__uploader" data-target-product-uploader></div>' +
        '<p class="target-product__upload-note" data-target-product-upload-counter>アップロードは任意です。ファイルを選択するとここに表示されます。</p>' +
        '</div>' +
        '</section>' +
        '</div>' +
        '<p class="user-management__feedback target-product__form-feedback" aria-live="polite" hidden></p>' +
        '<div class="target-product__form-actions target-product__form-row--full">' +
      ...
      var titleInput = form.querySelector('#target-product-title');
      var titleField = titleInput ? titleInput.closest('.target-product__form-field') : null;
      var descriptionInput = form.querySelector('#target-product-description');
      var startDateInput = form.querySelector('#target-product-start');
      var endDateInput = form.querySelector('#target-product-end');
      var linkInput = form.querySelector('#target-product-link');
      var authorField = form.querySelector('.target-product__author');
      if (authorField && authorField.closest('.target-product__form-field'))
      {
        authorField = authorField.closest('.target-product__form-field');
      }
      var authorActions = form.querySelector('[data-target-product-author-actions]');
      var authorSummary = form.querySelector('[data-target-product-author-summary]');
      var authorEmpty = form.querySelector('[data-target-product-author-empty]');
      var authorName = form.querySelector('[data-target-product-author-name]');
      var authorCode = form.querySelector('[data-target-product-author-code]');
      var authorSelectButton = null;
      var feedback = form.querySelector('.target-product__form-feedback');
      var uploaderHost = form.querySelector('[data-target-product-uploader]');
      var uploadCounter = form.querySelector('[data-target-product-upload-counter]');
      var uploadField = form.querySelector('.target-product__form-upload');
      var contentOpenButton = form.querySelector('[data-target-product-content-open]');
      var contentClearButton = form.querySelector('[data-target-product-content-clear]');
      var contentList = form.querySelector('[data-target-product-content-list]');
      var contentSummary = form.querySelector('[data-target-product-content-summary]');
      var contentPicker = form.querySelector('[data-target-product-content-picker]');
      var contentSearch = form.querySelector('[data-target-product-content-search]');
      var contentResults = form.querySelector('[data-target-product-content-results]');
      var contentFeedback = form.querySelector('[data-target-product-content-feedback]');
      var contentCloseButton = form.querySelector('[data-target-product-content-close]');
      var submitButton = form.querySelector('button[type="submit"]');

      if (authorActions)
      {
        authorSelectButton = this.createBannerButton('作成者を選択', 'target-product__author-select', {
          buttonType: 'content-uploader-primary',
          fallbackClass: 'btn btn--primary'
        });
        authorSelectButton.setAttribute('data-target-product-author-select', '');
      ...
      async function close(event)
      {
        ...
        {
          window.console.error('[target-detail] failed to confirm product modal close', error);
        }
      }
    ...
    async setupContentUploader(modal)
    {
      ...
      this.contentUploaderService = new window.Services.ContentUploader({
        ...
        {
          return self.uploadProductContent(file, options || {});
        },
    ...
    async submitProductForm(modal)
    {
      ...
      try
      {
        ...
        var apiMethod = isEditMode ? 'TargetProductUpdate' : 'TargetProductCreate';
        var response = await this.page.callApi(apiMethod, params, { requestType: 'TargetManagementProducts' });
        ...
        if (normalized)
        {
          ...
          if (isEditMode)
          {
            ...
            if (Array.isArray(this.page.state.products))
            {
              this.page.state.products = this.page.state.products.map(function (entry)
              {
                if (entry && entry.materialCode === normalized.materialCode)
                {
                  return normalized;
    ...
    uploadProductContent(file, options)
    {
      if (!file)
      {
      ...
      var fileName = file.name || 'product-upload';
  ...
  NS.JobProduct = NS.JobProduct || TargetDetailProduct;
})(window, document);
